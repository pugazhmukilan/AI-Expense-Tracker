...
class SmsService {
  ...
  static String _extractPersonName(String body) {
    ...
    final prioritizedPatterns = [
      ...
      RegExp(
        r'(?:from|by a/c linked to VPA)\s+([A-Za-z\s.\-]+?)(?:-|@|\(|UPI|Ref|is credited|credited|$)',
        caseSensitive: false,
      ),
      ...
      RegExp(
        r'debited for payee\s+([A-Za-z\s_]+?)\s+for\s+Rs',
        caseSensitive: false,
      ),
      ...
      RegExp(
        r'(?:to|from)\s+([A-Za-z\s]+?)\.?(?:\s*UPI|credited|debited|$)',
        caseSensitive: false,
      ),
      ...
      RegExp(
        r'^body\d+:\s*([A-Za-z0-9\s]+?)\s+has requested money',
        caseSensitive: false,
      ),
      ...
      RegExp(
        r'^body\d+:\s*([A-Za-z0-9\s]+?)\s+requested money|has requested',
        caseSensitive: false,
      ),
    ...
    for (var pattern in prioritizedPatterns) {
      ...
      if (match != null && match.groupCount >= 1) {
        ...
        name =
            name
                .replaceAll(RegExp(r'^(Mr|Ms|Dr)\s+', caseSensitive: false), '')
                .trim();
        ...
        if (name.isNotEmpty &&
            name.length > 2 &&
            !name.contains(RegExp(r'A/c|Acct|no\.', caseSensitive: false))) {
          return name;
        }
      }
    }
    ...
    if (vpaMatch != null && vpaMatch.groupCount >= 1) {
      ...
      if (vpaPart.isNotEmpty && vpaPart.length > 2) {
        ...
        if (vpaPart.contains(RegExp(r'[0-9]')) ||
            vpaPart.contains(RegExp(r'[A-Z]'))) {
    ...
    if (simpleMatch != null && simpleMatch.groupCount >= 1) {
      ...
      if (name.isNotEmpty &&
          name.length > 1 &&
          !name.contains(RegExp(r'SBI|IOB|ICICI', caseSensitive: false))) {
        return name;
      }
    }
